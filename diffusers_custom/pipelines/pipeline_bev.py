...
@dataclass
class BevPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class BevPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        only_inference_weight_dtype: torch.dtype, #torch.float32,
    ):
        super().__init__()

        if tokenizer is None and text_encoder is None:
            self.enable_text_inpipe = False
        else:
            self.enable_text_inpipe = True

        self.only_inference_weight_dtype = only_inference_weight_dtype
        ...
        **kwargs,
        ...
        if self.enable_text_inpipe:
            if lp_mask is None:
                ...
                text_embeddings = []
                for prompt in ['The street of the city.', 'vehicle', 'pedestrian']:
                    text_embeddings.append(self._encode_prompt(
                                        prompt, device, num_videos_per_prompt, do_classifier_free_guidance, negative_prompt
                                    ))
        else:
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            self.only_inference_weight_dtype, #text_embeddings.dtype, # torch.float32
            device,
            generator,
            latents,
        )
        ...
        skeleton = skeleton.to(latents.device)
        if preframe is not None:
            preframe = preframe.to(latents.device)
            ...
            for i, t in enumerate(timesteps):
                ...
                if preframe is not None:
                    if camera_ids is None:
                        noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings, lp_mask=lp_mask,
                                    skeleton=skeleton, preframe=preframe, train_or_sample='sample').sample.to(dtype=latents_dtype)
                    else:
                        noise_pred = self.unet(latent_model_input, t, view_idx=camera_ids, encoder_hidden_states=text_embeddings, lp_mask=lp_mask,
                                skeleton=skeleton, preframe=preframe, train_or_sample='sample').sample.to(dtype=latents_dtype)
                else:
                    if camera_ids is None:
                        noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings, lp_mask=lp_mask,
                                    skeleton=skeleton, train_or_sample='sample').sample.to(dtype=latents_dtype)
                    else:
                        noise_pred = self.unet(latent_model_input, t, view_idx=camera_ids, encoder_hidden_states=text_embeddings, lp_mask=lp_mask,
                                    skeleton=skeleton, train_or_sample='sample').sample.to(dtype=latents_dtype)
        ...
        return BevPipelineOutput(videos=video)
