    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None):
        ...
        if self.attn2 is not None:
            ...
            hidden_states = (
                ...
                )# norm_hidden_states 12 4096 320    encoder_hidden_states 6 77 768
...
class SparseCausalAttention(CrossAttention):#mk
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None):
        ...
        # former_frame_index[0] = 0 # [ 0 0 1 2 3 4]
                # self.camera_names = ['CAM_FRONT_LEFT','CAM_FRONT','CAM_FRONT_RIGHT','CAM_BACK_RIGHT','CAM_BACK','CAM_BACK_LEFT']

        former_frame_index = torch.tensor([5,0,1,2,3,4])
        posterior_frame_index = torch.tensor([1,2,3,4,5,0])

        key = rearrange(key, "(b f) d c -> b f d c", f=video_length)
        key = torch.cat([key[:, posterior_frame_index], key[:, former_frame_index]], dim=2)
        key = rearrange(key, "b f d c -> (b f) d c")

        value = rearrange(value, "(b f) d c -> b f d c", f=video_length)
        value = torch.cat([value[:, posterior_frame_index], value[:, former_frame_index]], dim=2)
