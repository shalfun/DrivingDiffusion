    ...
    def prepare_attention_mask(
        ...
        if current_length != target_length:
            if attention_mask.device.type == "mps":
                ...
                # attention_mask = F.pad(attention_mask, (0, target_length), value=0.0) # xf 
                attention_mask = attention_mask.unsqueeze(2).repeat(1, 1, target_length)
...
class XFormersAttnProcessor: #xf
    r"""
    Processor for implementing memory efficient attention using xFormers.
    ...
    def __call__(
        ...
        key = attn.to_k(encoder_hidden_states, *args) # 6 77 320
        value = attn.to_v(encoder_hidden_states, *args)

        query = attn.head_to_batch_dim(query).contiguous()# 48 4096 40
        key = attn.head_to_batch_dim(key).contiguous() # 48 77 40
        value = attn.head_to_batch_dim(value).contiguous()
        #xf
        if attention_mask is not None:
            attention_mask = F.pad(attention_mask, (0, 3), value=0.0)
            key = F.pad(key, (0, 0, 0, 3, 0, 0), value=0.0)
            value = F.pad(value, (0, 0, 0, 3, 0, 0), value=0.0)
            hidden_states = xformers.ops.memory_efficient_attention(
                query, key, value, attn_bias=attention_mask, op=self.attention_op, scale=attn.scale
            )
        else:
